...
import poly.edu.util.NumberToWordsConverter;
...
public class InventoryReceiptService {
    ...
    @Transactional
    public InventoryReceipt updateReceipt(Integer id, Integer supplierId, List<InventoryReceiptDetail> newDetails, String note) {
    ...
    public byte[] exportReceiptToExcel(Integer id) throws Exception {
        ...
        try(Workbook workbook = new XSSFWorkbook()) {
            ...
            dateCell.setCellValue(String.format("Ngày %02d tháng %02d năm %d", rTime.getDayOfMonth(), rTime.getMonthValue(), rTime.getYear()));
            ...
            // Info section
            int rowIdx = 8;
            sheet.createRow(rowIdx++).createCell(0).setCellValue("- Họ và tên người giao: " + receipt.getSupplier().getName());
            sheet.createRow(rowIdx++).createCell(0).setCellValue("- Theo hóa đơn số: ................ ngày ...... tháng ...... năm ...... của " + receipt.getSupplier().getName());
            sheet.createRow(rowIdx++).createCell(0).setCellValue("- Nhập tại kho: Kho của Shop OMG!   Địa điểm: ................................");

            // Table Header
            rowIdx++;
            Row th1 = sheet.createRow(rowIdx);
            th1.setHeight((short) 600);
            String[] headers = {"STT", "Tên, nhãn hiệu, quy cách,\nphẩm chất vật tư, dụng cụ\nsản phẩm, hàng hóa", "Mã số", "Đơn vị\ntính", "Số lượng", "", "Đơn giá", "Thành tiền"};
            ...
            for (InventoryReceiptDetail detail : receipt.getReceiptDetails()) {
                ...
                Cell c_stt = row.createCell(0); c_stt.setCellValue(stt++); c_stt.setCellStyle(tableCellStyle);
                Cell c_name = row.createCell(1); c_name.setCellValue(detail.getProductVariant().getProduct().getName() + " - " + detail.getProductVariant().getColor() + " - " + detail.getProductVariant().getSize()); c_name.setCellStyle(tableCellStyle);
                Cell c_sku = row.createCell(2); c_sku.setCellValue(detail.getProductVariant().getSku()); c_sku.setCellStyle(tableCellStyle);
                Cell c_dvt = row.createCell(3); c_dvt.setCellValue("Cái"); c_dvt.setCellStyle(tableCellStyle);
                
                Cell c_sl1 = row.createCell(4); c_sl1.setCellValue(detail.getQuantity()); c_sl1.setCellStyle(tableCellStyle);
                Cell c_sl2 = row.createCell(5); c_sl2.setCellValue(detail.getQuantity()); c_sl2.setCellStyle(tableCellStyle);
                
                Cell c_price = row.createCell(6); c_price.setCellValue(detail.getImportPrice() != null ? detail.getImportPrice() : 0); c_price.setCellStyle(numCellStyle);
                
                double amount = detail.getQuantity() * (detail.getImportPrice() != null ? detail.getImportPrice() : 0);
                sumTotal += amount;
                Cell c_amount = row.createCell(7); c_amount.setCellValue(amount); c_amount.setCellStyle(numCellStyle);
            }
            ...
            // Text Amount
            rowIdx++;
            sheet.createRow(rowIdx++).createCell(0).setCellValue("- Tổng số tiền (Viết bằng chữ): " + NumberToWordsConverter.convert((long) sumTotal));
            sheet.createRow(rowIdx++).createCell(0).setCellValue("- Số chứng từ gốc kèm theo: ..............................................................");

            // Signatures
            rowIdx++;
            Row signDateRow = sheet.createRow(rowIdx++);
            Cell cDateSign = signDateRow.createCell(5);
            cDateSign.setCellValue(String.format("Ngày %02d tháng %02d năm %d", rTime.getDayOfMonth(), rTime.getMonthValue(), rTime.getYear()));
            ...
            Cell s1 = signRow.createCell(1); s1.setCellValue("Người lập phiếu"); s1.setCellStyle(boldStyle);
            Cell s2 = signRow.createCell(5); s2.setCellValue("Người giao hàng"); s2.setCellStyle(boldStyle);

            Row signDescRow = sheet.createRow(rowIdx++);
            Cell sd1 = signDescRow.createCell(1); sd1.setCellValue("(Ký, họ tên)"); sd1.setCellStyle(italicCenterStyle);
            Cell sd2 = signDescRow.createCell(5); sd2.setCellValue("(Ký, họ tên)"); sd2.setCellStyle(italicCenterStyle);
            
            // Name of creator
            rowIdx += 4;
            Row nameRow = sheet.createRow(rowIdx);
            Cell n1 = nameRow.createCell(1); n1.setCellValue(receipt.getAccount().getFullName()); n1.setCellStyle(centerStyle);
            Cell n2 = nameRow.createCell(5); n2.setCellValue(receipt.getSupplier().getName()); n2.setCellStyle(centerStyle);
